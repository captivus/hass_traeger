    ...
    for probe_type in ['wired', 'bluetooth', 'legacy']:
        ...
        if model_path.exists():
            try:
                ...
                logger.info(f"Loaded pre-trained {probe_type} model")
            except Exception as e:
                logger.warning(f"Failed to load {probe_type} model: {e}")
...
def predict(cook_id: str, probe_channel: str):
    ...
    # For early predictions, try pre-trained model first
    probe_type = get_probe_type(probe_channel)
    if len(temps) < 20 and probe_type in PRETRAINED_MODELS:
        model = PRETRAINED_MODELS[probe_type]
        minutes, message = xgboost_prediction_improved(model, times, temps, targets, grill_temps)
        if minutes is not None:
            return {'minutes_to_target': minutes, 'message': f"{message} (pre-trained)", 
                    'method': 'xgboost_pretrained',
                    'current_temp': current_temp, 'target_temp': target_temp, 
                    'data_points': len(temps)}
    
    # Try cook-specific XGBoost if we have enough data
